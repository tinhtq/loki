{
  ...
  _config+:: {
    bloom_compactor+: {
      pvc_size: '256Gi',  // error 'define bloom_compactor.pvc_size',
      pvc_class: 'fast',  // error 'define bloom_compactor.pvc_class',
    },
    loki+: {
      bloom_compactor+: {
  ...
  bloom_compactor_statefulset:
    if $._config.use_bloom_filters
    then
      statefulSet.new(name, 3, [$.bloom_compactor_container], $.bloom_compactor_data_pvc)
